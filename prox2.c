...
static void sizechanged() {
  ...
  ssh_channel_change_pty_size(chan_tmp, win.ws_col, win.ws_row);
...
static void select_loop(ssh_session session, ssh_channel channel_1, ssh_channel channel_2, socket_t sock)
{
  fd_set fds;
...
static void shell(ssh_session session, ssh_channel channel, socket_t sock)
{
  ...
  if (interactive)
    {
      ssh_channel_request_pty(channel_remote);
      sizechanged();
    }

  printf("après channel request pty\n");

  if (ssh_channel_request_shell(channel_remote))
    {
  ...
  printf("après signal\n");
  select_loop(session, channel, channel_remote, sock);
...
static void batch_shell(ssh_session session, ssh_channel channel, socket_t sock)
{
  ...
  if (ssh_channel_request_exec(channel_remote, buffer))
    {
